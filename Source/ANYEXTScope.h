...
#define onExit \
    any_keywordify \
    __strong any_cleanupBlock_t metamacro_concat(any_exitBlock_, __LINE__) __attribute__((cleanup(any_executeCleanupBlock), unused)) = ^
#endif
...
#define weakify(...) \
    any_keywordify \
    metamacro_foreach_cxt(any_weakify_,, __weak, __VA_ARGS__)
#endif
...
#define unsafeify(...) \
    any_keywordify \
    metamacro_foreach_cxt(any_weakify_,, __unsafe_unretained, __VA_ARGS__)
#endif
...
#define strongify(...) \
    ...
    metamacro_foreach(any_strongify_,, __VA_ARGS__) \
...
#define any_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define any_strongify_(INDEX, VAR) \
...
// autorelease pools.
#if DEBUG
#define any_keywordify autoreleasepool {}
#else
#define any_keywordify try {} @catch (...) {}
#endif
